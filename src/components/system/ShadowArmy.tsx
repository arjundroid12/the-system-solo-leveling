        ...
        <div className="p-4">
          {shadows.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div><p className="font-display text-xl sl-glow-shadow">{shadows.length}</p><p className="text-[10px] text-[var(--system-text-dim)] tracking-widest mt-1">TOTAL</p></div>
              <div><p className="font-display text-xl sl-glow-blue">{deployedCount}</p><p className="text-[10px] text-[var(--system-text-dim)] tracking-widest mt-1">DEPLOYED</p></div>
              <div><p className="font-display text-xl sl-glow-red">{totalAttack}</p><p className="text-[10px] text-[var(--system-text-dim)] tracking-widest mt-1">ATK</p></div>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="p-3 space-y-2">
            {shadows.map((shadow, i) => (
              <div key={shadow.id} className="border border-[var(--system-border)] p-3 sl-shadow-rise" style={{ animationDelay: `${i * 0.05}s` }}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between">
                      <p className="font-display text-sm sl-glow-shadow">{shadow.name} {shadow.deployed && <span className="text-[10px] sl-glow-blue">[DEPLOYED]</span>}</p>
                    ...
                    <p className="text-[10px] text-[var(--system-text-dim)] mt-0.5">{shadow.originalName} · LV {shadow.level} · {shadow.ability}</p>
                ...
                <p className="text-[10px] text-[var(--system-text-dim)] mt-1 text-center">Deployed shadows gain XP when you clear dungeons</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-3 space-y-2">
          {skills.filter(s => s.category === 'SHADOW').map(skill => (
            <div key={skill.id} className={`flex items-center gap-3 p-2 border ${skill.unlocked ? 'border-[var(--system-shadow)]/40' : 'border-[var(--system-border)] opacity-40'}`}>
              ...
              <div className="flex-1">
                <p className={`text-xs ${skill.unlocked ? 'sl-glow-shadow' : 'text-[var(--system-text-dim)]'}`}>{skill.name} <span className="text-[10px]">[{skill.rank}]</span></p>
                <p className="text-[10px] text-[var(--system-text-dim)]">{skill.description}</p>
